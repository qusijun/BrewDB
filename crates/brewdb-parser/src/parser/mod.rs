...
pub use crate::dialect::derive_dialect;
#[cfg(test)]
pub use crate::test_utils;
pub use crate::{ast, dialect, display_utils, keywords, tokenizer};
...
impl<'a> Parser<'a> {
    ...
    /// Example:
    /// ```
    /// # use brewdb_parser::{parser::{Parser, ParserError}, dialect::GenericDialect};
    ...
    /// Example:
    /// ```
    /// # use brewdb_parser::{parser::{Parser, ParserError, ParserOptions}, dialect::GenericDialect};
    /// # fn main() -> Result<(), ParserError> {
    /// let dialect = GenericDialect{};
    ...
    /// Parses fulltext expressions [`brewdb_parser::ast::Expr::MatchAgainst`]
    ///
    /// # Errors
    ...
    /// Example:
    /// ```rust
    /// # use brewdb_parser::dialect::GenericDialect;
    /// # use brewdb_parser::parser::Parser;
    /// # use brewdb_parser::keywords::Keyword;
    /// # use brewdb_parser::tokenizer::{Token, Word};
    /// let dialect = GenericDialect {};
    ...
    pub fn parse_create_table(
        ...
        let partitioned_by = self.parse_optional_partitioned_by()?;
        let hive_distribution = self.parse_hive_distribution()?;
        let clustered_by = self.parse_optional_clustered_by()?;
        let distributed_by = self.parse_optional_distributed_by()?;
        let hive_formats = self.parse_hive_formats()?;
    ...
    fn parse_optional_partitioned_by(&mut self) -> Result<Vec<Ident>, ParserError> {
        if !self.parse_keywords(&[Keyword::PARTITIONED, Keyword::BY]) {
            return Ok(vec![]);
        }
        ...
        if columns.is_empty() {
            return parser_err!(
                "PARTITIONED BY must specify at least one column",
                self.peek_token_ref()
            );
        }
        Ok(columns)
    ...
    fn parse_optional_distributed_by(&mut self) -> Result<Option<DistributedBy>, ParserError> {
        if !self.parse_keyword(Keyword::DISTRIBUTED) {
            return Ok(None);
        }

        if self.parse_keyword(Keyword::INTO) {
            let buckets = self.parse_literal_uint()?;
            self.expect_keyword(Keyword::BUCKETS)?;
            return Ok(Some(DistributedBy {
                function: None,
                columns: vec![],
        ...
        let function = if !self.peek_token_ref().token.eq(&Token::LParen) {
            Some(self.parse_identifier()?)
        } else {
            None
        };
        ...
        if columns.is_empty() {
            return parser_err!(
                "DISTRIBUTED BY must specify at least one column",
                self.peek_token_ref()
            );
        }

        let buckets = if self.parse_keyword(Keyword::INTO) {
            let buckets = self.parse_literal_uint()?;
            self.expect_keyword(Keyword::BUCKETS)?;
            Some(buckets)
        } else {
    ...
    fn parse_optional_create_table_config(
        ...
        if dialect_of!(self is BigQueryDialect | GenericDialect | PostgreSqlDialect) {
            if self.parse_keywords(&[Keyword::CLUSTER, Keyword::BY]) {
                cluster_by = if self.consume_token(&Token::LParen) {
                    let columns = self.parse_comma_separated(|p| p.parse_expr())?;
                    self.expect_token(&Token::RParen)?;
                    Some(WrappedCollection::Parentheses(columns))
                } else {
    ...
    pub fn parse_copy(&mut self) -> Result<Statement, ParserError> {
        if self.parse_keyword(Keyword::FROM) {
            let source = CopySource::File {
                filename: self.parse_literal_string()?,
            };
            self.expect_keyword(Keyword::TO)?;
            let table_name = self.parse_object_name(false)?;
            let columns = self.parse_parenthesized_column_list(Optional, false)?;
            let target = CopyTarget::Table {
                table_name,
                columns,
            };
    ...
    pub fn parse_show(&mut self) -> Result<Statement, ParserError> {
        ...
        } else if self.parse_keyword(Keyword::CATALOGS) || self.parse_keyword(Keyword::CATALOG) {
            self.parse_show_catalogs(terse)
        } else if self.parse_keyword(Keyword::DATABASES) || self.parse_keyword(Keyword::DATABASE) {
            self.parse_show_databases(terse)
        } else if self.parse_keyword(Keyword::SCHEMAS) || self.parse_keyword(Keyword::SCHEMA) {
    ...
    /// Example
    /// ```
    /// # use brewdb_parser::parser::{Parser, ParserError};
    /// # use brewdb_parser::dialect::GenericDialect;
    /// # fn main() ->Result<(), ParserError> {
...
mod tests {
    ...
    #[test]
    fn parses_create_table_partitioned_by_clause() {
        let statements = Parser::parse_sql(
            &crate::dialect::GenericDialect {},
            "create table t (id int, dt text, region text) partitioned by (dt, region)",
        )
        .unwrap();
        ...
        match &statements[0] {
            Statement::CreateTable(create_table) => {
                let columns = create_table
                    ...
                    .collect::<Vec<_>>();
                assert_eq!(columns, vec!["dt", "region"]);
            }
    ...
    #[test]
    fn parses_copy_from_file_to_table() {
        let statements = Parser::parse_sql(
            &crate::dialect::GenericDialect {},
            "copy from '/tmp/orders.csv' to orders with (format csv, header true)",
        )
        .unwrap();
        ...
        match &statements[0] {
            Statement::Copy {
                ...
                assert!(matches!(
                    source,
                    CopySource::File { filename } if filename == "/tmp/orders.csv"
                ));
                assert!(matches!(
                    target,
                    CopyTarget::Table { table_name, columns }
                        if table_name.to_string() == "orders" && columns.is_empty()
                ));
