...
pub struct CreateTable {
    ...
    /// BrewDB/Paimon `PARTITIONED BY (col_name, ...)` clause.
    ...
    /// BrewDB/Paimon distribution metadata.
    pub distributed_by: Option<DistributedBy>,
}

/// Flink-style `DISTRIBUTED BY [function](col_name, ...) [INTO n BUCKETS]`
/// or `DISTRIBUTED INTO n BUCKETS` clause.
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct DistributedBy {
    /// Optional bucket function name, for example `hash` in `DISTRIBUTED BY hash(id)`.
    pub function: Option<Ident>,
    /// Bucket key column names.
    pub columns: Vec<Ident>,
    /// Optional fixed bucket count.
    pub buckets: Option<u64>,
}

impl fmt::Display for DistributedBy {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if self.columns.is_empty() {
            f.write_str("DISTRIBUTED")?;
            if let Some(buckets) = self.buckets {
                write!(f, " INTO {buckets} BUCKETS")?;
            }
            return Ok(());
        }

        f.write_str("DISTRIBUTED BY ")?;
...
impl fmt::Display for CreateTable {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(distributed_by) = &self.distributed_by {
            write!(f, " {distributed_by}")?;
        }
        if !self.partitioned_by.is_empty() {
            write!(
                f,
                " PARTITIONED BY ({})",
                display_comma_separated(&self.partitioned_by)
            )?;
        }
