...
pub use self::ddl::{
    ...
    DistributedBy, DropBehavior, DropExtension, DropFunction, DropOperator, DropOperatorClass,
...
/// ```
/// # use brewdb_parser::ast::OneOrManyWithParens;
            ...
            Statement::Copy {
                ...
                match source {
                    CopySource::File { filename } => {
                        write!(f, " FROM '{}'", value::escape_single_quote_string(filename))?
                    }
                ...
                if matches!(source, CopySource::File { .. }) {
                    write!(f, " TO {target}")?;
                } else {
...
pub enum CopyTarget {
    /// Copy into a table with optional column list.
    Table {
        /// The name of the table to copy into.
        table_name: ObjectName,
        /// A list of column names to copy. Empty list means all columns.
        columns: Vec<Ident>,
    },
...
/// # Examples
/// ```
/// # use brewdb_parser::ast::WrappedCollection;
