...
use crate::catalog::model::{CatalogEntry, DatabaseCatalogEntry, StorageKind, TableCatalogEntry};
use crate::catalog::paimon_schema::PaimonSchemaAdapter;
use crate::catalog::path::{DatabasePath, TablePath};
...
impl Catalog for ManagedPaimonCatalog {
    ...
    fn create_table(&self, request: CreateTableRequest) -> Result<TableCatalogEntry, CatalogError> {
        if self.entry.storage_kind != StorageKind::Paimon {
            return Err(CatalogError::CatalogFormatMismatch {
                catalog: self.entry.path.catalog().to_owned(),
                expected: StorageKind::Paimon.as_str(),
                actual: self.entry.storage_kind.as_str(),
            });
        }
...
mod tests {
    ...
    fn mock_catalog() -> ManagedPaimonCatalog {
        ...
        let schema = Schema::builder()
            ...
            .column(
                "dt",
                paimon::spec::DataType::VarChar(paimon::spec::VarCharType::new(255).unwrap()),
            )
            .partition_keys(["dt"])
            .primary_key(["id"])
            .option("bucket", "1")
    ...
    fn managed_paimon_catalog_creates_entries_in_store() {
        ...
        assert_eq!(table.table_schema.fields.len(), 2);
        assert_eq!(table.table_schema.primary_keys, vec!["id"]);
        assert_eq!(table.table_schema.partition_keys, vec!["dt"]);
        assert_eq!(table.table_schema.bucket_keys, vec!["id"]);
        assert_eq!(
