...
use crate::{SqlError, SqlRequestContext, SqlSessionContext, Statement};
...
use self::explain::bind_explain_statement;
use self::mutation::{
...
use self::query::bind_query_statement;
use self::session::{bind_set_statement, bind_use_statement};
...
pub(super) fn planner_to_sql_error(error: PlannerError) -> SqlError {
    match error {
...
pub(super) fn empty_df_schema() -> DFSchemaRef {
    Arc::new(DFSchema::empty())
}

pub(super) fn extension_plan(node: LogicalPlanNode) -> DataFusionLogicalPlan {
    DataFusionLogicalPlan::Extension(Extension {
        node: Arc::new(node),
    })
}

pub(super) fn qualify_database_name(
...
pub(super) fn resolve_table_object(
    ctx: &LogicalPlanningContext<'_>,
    session: &LogicalPlanningSession,
    table: &TableObject,
) -> Result<TableCatalogEntry, SqlError> {
...
pub(super) fn resolve_query_tables(
    ctx: &LogicalPlanningContext<'_>,
    session: &LogicalPlanningSession,
...
pub(super) fn name_parts(name: &ObjectName) -> Result<Vec<String>, SqlError> {
    name.0
        .iter()
