//! Worker-side exchange data flow.
...
mod tests {
    ...
    fn channel(exchange_id: ExchangeId, exchange_type: ExchangeType) -> ExchangeChannelDescriptor {
        ExchangeChannelDescriptor {
            ...
            target_endpoint: "rpc://worker-0".to_owned(),
            scope: ExchangeScope::Local,
            exchange_type,
            partitioning_scheme: PartitioningScheme::single(),
        }
    }
    ...
    fn exchange_buffer_manager_drains_batches_by_exchange_id() {
        let channel = channel(ExchangeId(0), ExchangeType::Gather);
    ...
    fn exchange_batch_router_repartitions_rows_by_hash_key() {
        let mut channels = vec![ExchangeChannelDescriptor {
            partitioning_scheme: PartitioningScheme::hash([
                datafusion_common::Column::new_unqualified("id"),
            ]),
            ..channel(ExchangeId(0), ExchangeType::Repartition)
        }];
